...
//                                                              
//  transformDailyData  now maps the dedicated daily-summary
//  endpoint response (shape: { dailySummary: [...] })
//                                                              
function transformDailyData(json) {
  if (!json || !Array.isArray(json.dailySummary)) return [];
  return json.dailySummary
    .map(row => {
      ...
      return {
        ...
        femalePercent: row.femalePercent ?? null,
        peakHour: row.peakHour ?? null,
        cashierAlerts: mapAlerts(row.cashierAlerts ?? []),
        securityAlerts: mapAlerts(row.securityAlerts ?? []),
      };
    })
    .sort((a, b) => (a.date < b.date ? 1 : -1));
}
              ...
              {visible.length === 0 ? (
                ...
                return (
                  <tr key={row.date} className={weekend ? 'daily-row--weekend' : ''}>
                    ...
                    <td>
                      <span className="daily-cell-peak">
                        {row.peakHour ? <><Clock size={9} />{row.peakHour}</> : ''}
                      </span>
                    </td>
...
export default function Analytics() {
  ...
  const [dailyRows, setDailyRows] = useState([]);
  const [dailyRange, setDailyRange] = useState(30);
  // track whether we're loading the daily summary separately
  ...
  //                                                          
  //  fetchDailySummary   calls the dedicated endpoint with
  //  the current dailyRange (7 or 30) so data actually comes
  //  from the DB for the selected window.
  //                                                          
  const fetchDailySummary = useCallback(async () => {
    ...
    try {
      const token = getToken();
      const storeParam = activeStore ? `&store=${encodeURIComponent(activeStore)}` : '';
      const url = `${API_BASE}/api/analytics/daily-summary?date=${activeDate}&days=${dailyRange}${storeParam}`;
  ...
  }, [activeDate, activeStore, dailyRange]); // <-- dailyRange is now a dependency
  ...
  /*  (re)load daily summary when date, store, OR range changes  */
  useEffect(() => {
    fetchDailySummary();
  }, [fetchDailySummary]); // fetchDailySummary already captures dailyRange
  ...
  return (
    <div className="an-page">
      ...
      <DailyBreakdownTable dailyRows={dailyRows} />
    </div>
  );
}